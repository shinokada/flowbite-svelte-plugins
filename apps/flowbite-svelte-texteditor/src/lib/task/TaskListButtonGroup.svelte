...
{#if editor && showToolbar}
  {#if toggle}
    <TaskListButton {editor} action="toggle" />
  {/if}
  {#if split}
    <TaskListButton {editor} action="split" />
  {/if}
  {#if sink}
    <TaskListButton {editor} action="sink" />
  {/if}
  {#if lift}
...
## Type
[TaskListButtonGroupProps](https://github.com/shinokada/flowbite-svelte-plugins/blob/main/src/lib/types.ts#L209)
## Props
@prop editor
