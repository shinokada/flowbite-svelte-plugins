<script lang="ts">
  ...
  import { runInvisibleCommand, type InvisibleAction, cn, generateButtonId } from '$lib';
  import { type InvisibleButtonProps } from '$lib/types';
...
<button
  ...
  id={uniqueId}
  type="button"
  class={cn('cursor-pointer rounded-sm p-1.5', isActive ? 'bg-gray-200 text-gray-900 dark:bg-gray-600 dark:text-white' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-white', className)}
>
  {#if action === 'toggle'}
...
## Type
[InvisibleButtonProps](https://github.com/shinokada/flowbite-svelte-plugins/blob/main/src/lib/types.ts#L135)
## Props
@prop editor
@prop action
@prop tooltipText
