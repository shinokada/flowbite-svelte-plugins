<script lang="ts">
  ...
  let { editor, headingsOnly = false, showHeadings = true, showList = true, showCodeBlock = true, showQuote = true, showHorizontalRule = true, showTable = true, showTasks = true, showImage = true, autoSetup = true }: FloatingMenuProps & { autoSetup?: boolean } = $props();
...
<div class="floating-menu" data-testid="floating-menu">
  {#if showHeadings}
    <button onclick={() => runHeadingCommand(editor, 1)} class:is-active={isHeading1Active} title="Heading 1" type="button"> H1 </button>
    <button onclick={() => runHeadingCommand(editor, 2)} class:is-active={isHeading2Active} title="Heading 2" type="button"> H2 </button>
    <button onclick={() => runHeadingCommand(editor, 3)} class:is-active={isHeading3Active} title="Heading 3" type="button"> H3 </button>
  {/if}

  {#if !headingsOnly && showList}
    <button onclick={() => runListCommand(editor, 'bullet')} class:is-active={isBulletListActive} title="Bullet list" aria-label="Bullet list" type="button">
    ...
    <button onclick={() => runListCommand(editor, 'ordered')} class:is-active={isOrderedListActive} title="Ordered List" aria-label="Ordered list" type="button">
      <svg class="h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
  ...
  {#if !headingsOnly && showCodeBlock}
    <button onclick={() => runLayoutsCommand(editor, 'codeblock')} class:is-active={isCodeBlockActive} title="Code Block" type="button"> Code </button>
  {/if}

  {#if !headingsOnly && showQuote}
    <button onclick={() => runLayoutsCommand(editor, 'blockquote')} class:is-active={isBlockquoteActive} title="Blockquote" type="button"> Quote </button>
  {/if}

  {#if !headingsOnly && showHorizontalRule}
    <button onclick={() => runLayoutsCommand(editor, 'hr')} title="Horizontal Rule" type="button"> HR </button>
  {/if}

  {#if !headingsOnly && showTasks}
    <button onclick={() => runTaskCommand(editor, 'toggle')} title="Task List" type="button"> Tasks </button>
  {/if}

  {#if !headingsOnly && showTable}
    <button onclick={() => runTableCommand(editor, 'table')} title="Insert Table" type="button"> Table </button>
  {/if}

  {#if !headingsOnly && showImage}
    <button onclick={() => runImageCommand(editor, 'basic')} title="Insert Image" type="button"> Image </button>
  {/if}
</div>
